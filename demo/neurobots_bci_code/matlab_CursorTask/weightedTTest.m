function p = weightedTTest(x,m, decay, alpha,tail,dim)
...
weightsForX = decay .^ linspace(numel(x) - 1, 0, numel(x));
% hack new attempt
%weightsForX = log(linspace(2, numel(x) + 1, numel(x)));
% normalize in some weird way :D
weightsForX =  weightsForX .* (numel(x) / sum(weightsForX));
xmean = nanmean((x .* weightsForX),dim);
diffs = x - xmean;
weightedDiffsToMean = diffs .* weightsForX;
sdpop = nanstd(weightedDiffsToMean,[],dim);
